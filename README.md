...
**Migration:** the CENTRAL broker (the upstream a replica points `broker_url` at) must now also carry `serve_replicas: true` (or `CLAUDE_PEERS_SERVE_REPLICAS=1`) -- without it, an otherwise correctly configured upstream refuses to serve the `/roadmap/sync/pull|push|lock` and `/federation/sync|send` routes, and **every replica reports it unreachable**, indistinguishable from a network outage, even with `broker_token` set correctly.

Scope of this mode:

- `roadmap_items` is replicated, and while the link is up `peers` and `messages` are federated: the replica relays its agents to the upstream on every sync pass (`POST /federation/sync`, a heartbeat for all of them at once) and relays each message to a remote peer synchronously (`POST /federation/send`), so agents on this machine see the upstream's peers and the other replicas' peers in `list_peers` (`Via: upstream broker`) and message them as in Mode 2, without any client leaving loopback. During an outage the remote peers stay visible for `CLAUDE_PEERS_FEDERATION_GRACE_SEC` (default 600 s) with a `link down for Ns` mention and a message sent to one of them is queued (`queued: true` in the ack) and delivered at reconnection. Past the grace, remote peers disappear from `list_peers`, the queue is dropped and each sender is told so by a local message from `deck`; messaging between local agents is never affected. Inbound messages sent to this machine's agents during the cut wait upstream and arrive at the first pass after reconnection.
- The upstream must run a version serving `/federation/sync|send` for cross-machine messaging: an older upstream (404 on those routes) gives roadmap replication only -- `list_peers` stays local and `/health` reports `federation: 'unsupported'`.
- Approvals, dispatch, graph drafts and the operator inbox stay local to each broker. The operator inbox and Deck announcements never leave the machine, online or offline: `send_message('operator')` is always a local deposit.
...
across two brokers that only meet through the replication pass (its tick,
its federation grace). Put the `offline_replica` flag in the shared config file
...
| `CLAUDE_PEERS_ACTIVE_STALE_SEC`      | (n/a)                  | `120`                                | broker                | Seconds without heartbeat before an active peer is swept to dormant; a mirror row (a replica's copy of an upstream peer, with no heartbeat of its own) is floored to twice the longest interval between two sync passes (the 60s failure backoff ceiling, or `CLAUDE_PEERS_SYNC_TICK_MS` if larger) so a late pass cannot flip it dormant mid-grace; a replica started below that floor logs one warning |
...
| `CLAUDE_PEERS_SYNC_TICK_MS`          | (n/a)                  | `5000`                               | broker                | Mode 3 only: cadence of the replica's pull/push/lock/federation sync pass against its upstream. |
| `CLAUDE_PEERS_FEDERATION_GRACE_SEC`  | (n/a)                  | `600` (10 min)                       | broker                | Mode 3 only, seconds (floor 1, a tunable meant for tests below 30): after the upstream link drops, how long remote peers stay visible in `list_peers` and messages to them are queued; past it the mirrors go dormant, the queue is dropped and each sender is notified by a local message from `deck`. |
